...
let usageCache:
	| { session: ExtensionContext["sessionManager"]; key: string; totals: UsageTotals }
	| undefined;
...
export function getUsageTotals(ctx: ExtensionContext): UsageTotals {
	...
	if (usageCache?.session === ctx.sessionManager && usageCache.key === key)
	...
	usageCache = { session: ctx.sessionManager, key, totals };
	return totals;
}
