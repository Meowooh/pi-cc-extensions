...
const RUNTIMES: readonly RuntimeDef[] = [
	{
		...
		versionCommand: { cmd: "bun", args: ["--version"], pattern: /(\d+\.\d+\.\d+)/ },
	},
	{
		name: "nodejs",
		files: ["package.json", ".nvmrc", ".node-version"],
		versionCommand: { cmd: "node", args: ["--version"], pattern: /v(\d+\.\d+\.\d+)/ },
	},
	{
		name: "rust",
		files: ["Cargo.toml"],
		versionCommand: { cmd: "rustc", args: ["--version"], pattern: /rustc\s+(\d+\.\d+\.\d+)/ },
	},
	{
		name: "go",
		files: ["go.mod"],
		versionCommand: { cmd: "go", args: ["version"], pattern: /go(\d+\.\d+\.\d+)/ },
	},
	{
		name: "python",
		files: ["pyproject.toml", "requirements.txt", "setup.py", "Pipfile", ".python-version"],
		versionCommand: { cmd: "python3", args: ["--version"], pattern: /Python\s+(\d+\.\d+\.\d+)/ },
	},
	{
		name: "ruby",
		files: ["Gemfile", ".ruby-version"],
		versionCommand: { cmd: "ruby", args: ["--version"], pattern: /ruby\s+(\d+\.\d+\.\d+)/ },
	},
	{
		name: "java",
		files: ["pom.xml", "build.gradle", "build.gradle.kts", ".java-version"],
		versionCommand: { cmd: "java", args: ["-version"], pattern: /version\s+"(\d+\.\d+[\.\d]*)"/ },
	},
	{
		name: "swift",
		files: ["Package.swift"],
		versionCommand: { cmd: "swift", args: ["--version"], pattern: /Swift\s+(\d+\.\d+)/ },
	},
	...
	{ name: "c", files: ["Makefile", "CMakeLists.txt"] },
	{
		...
		versionCommand: { cmd: "deno", args: ["--version"], pattern: /deno\s+(\d+\.\d+\.\d+)/ },
	},
	{
		name: "php",
		files: ["composer.json"],
		versionCommand: { cmd: "php", args: ["--version"], pattern: /PHP\s+(\d+\.\d+\.\d+)/ },
	},
	{
		name: "haskell",
		files: ["stack.yaml", "cabal.project", ".cabal"],
		versionCommand: { cmd: "ghc", args: ["--version"], pattern: /(\d+\.\d+\.\d+)/ },
	},
	{
		name: "julia",
		files: ["Project.toml", "Manifest.toml"],
		versionCommand: { cmd: "julia", args: ["--version"], pattern: /julia\s+(\d+\.\d+\.\d+)/ },
	},
	{
		name: "lua",
	...
	},
	{
		name: "elixir",
		files: ["mix.exs"],
		versionCommand: { cmd: "elixir", args: ["--version"], pattern: /Elixir\s+(\d+\.\d+\.\d+)/ },
	},
	{ name: "erlang", files: ["rebar.config", "erlang.mk"] },
	{
		name: "gleam",
		files: ["gleam.toml"],
		versionCommand: { cmd: "gleam", args: ["--version"], pattern: /gleam\s+(\d+\.\d+\.\d+)/ },
	},
	{
		name: "crystal",
		files: ["shard.yml"],
		versionCommand: { cmd: "crystal", args: ["--version"], pattern: /Crystal\s+(\d+\.\d+\.\d+)/ },
	},
	{
		name: "dart",
		files: ["pubspec.yaml"],
	...
	{ name: "nim", files: ["nim.cfg", ".nimble"] },
	{
		name: "zig",
		files: ["build.zig"],
		versionCommand: { cmd: "zig", args: ["version"], pattern: /(\d+\.\d+\.\d+)/ },
	},
