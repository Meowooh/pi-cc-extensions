...
function renderContextCompact(theme: Theme, ctx: ExtensionContext, glyphs: IconGlyphs): string {
	...
	const unknown = contextUsage?.percent === null;
	const color = unknown ? "muted" : stressColor(contextPct);
	return `${theme.fg(color, glyphs.context)} ${theme.fg(color, unknown ? "?%" : `${contextPct.toFixed(1)}%`)}`;
}
...
function renderTimerSegment(theme: Theme, state: FooterState, glyphs: IconGlyphs): string {
	...
	if (!run) return "";
	const color = run.complete ? "success" : "accent";
	return `${theme.fg(color, run.complete ? glyphs.done : glyphs.working)} ${theme.fg(run.complete ? "success" : "dim", run.complete ? "done" : "working")} ${theme.fg(run.complete ? "text" : "accent", run.duration)}`;
}
...
function renderContextBar(
	...
	if (contextUsage?.percent === null) {
		return `${theme.fg("muted", `${glyphs.context} [?] ?%`)} ${theme.fg("dim", "·")} ${theme.fg("text", `?/${fmtTokens(contextWindow)}`)}`;
	}
...
function renderStatsBlock(
	...
	if (segments.tokens) {
		stats.push(
			theme.fg("accent", `${glyphs.input} ${formatInputBreakdown(totals.input, totals.cacheRead)}`),
		);
...
export function installFooter(
	...
	ctx.ui.setFooter((tui, activeTheme, footerData) => {
		...
		return {
			...
			render(width: number): string[] {
				...
				const savedTotals = getUsageTotals(ctx);
				const totals = {
					...savedTotals,
					output: Math.max(
						savedTotals.output,
						state.outputBaseline + (state.runSummary?.tokens ?? 0),
					),
				};
	...
	return () => {
		if (ownsFooter) ctx.ui.setFooter(undefined);
	};
}
